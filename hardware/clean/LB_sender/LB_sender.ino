...
void check_receive()
{
  ...
  /* Check header */
  bool found_header;
  found header = radio.read( READ_HEAD_BUF, sizeof(lightscript_header));
  ...
  READ_BUF = realloc_read_buf(READ_BUF, script_len);
  ...
  while (!done)
  {
    ...
    done = radio.read(&temp_buf, sizeof(unsigned long));
  ...
  /* Send ACK */
  radio.stopListening();
  int temp_test_num = 42;
  radio.write(&temp_test_num, sizeof(uint8_t));
...
void send_LB()
{
  ...
  Serial.println("Now listening for response...");
  radio.startListening();
...
blinkm_script_line **realloc_read_buf(blinkm_script_line **orig_ptr, uint8_t script_len)
{
